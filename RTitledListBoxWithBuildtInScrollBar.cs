...
namespace RTheme
{
    public class RTitledListBoxWithBuildtInScrollBar : Control
    {
        ...
        public RTitledListBoxWithBuildtInScrollBar()
        {
            __ENCAddToList(this);
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            checked
            {
                ...
                while (true)
                {
                    ...
                    LogInListBoxItem logInListBoxItem = Items[num5];
                    int num8 = ItemHeight + num5 * ItemHeight + 1 - num + (int)Math.Round((double)ItemHeight / 2.0 - 8.0);
                    if (_SelectedItems.Contains(logInListBoxItem))
                    {
                        ...
                        rect = new Rectangle(0, ItemHeight + num5 * ItemHeight + 1 - num, Width - 19, ItemHeight - 1);
                    ...
                    graphics2.DrawLine(new Pen(_BorderColour), 0, ItemHeight + num5 * ItemHeight + 1 - num + ItemHeight - 1, Width - 18, ItemHeight + num5 * ItemHeight + 1 - num + ItemHeight - 1);
                    graphics2.DrawString(logInListBoxItem.Text, new Font("Segoe UI", 8f), new SolidBrush(_TextColour), 9f, num8);
                    graphics2.ResetClip();
                    num5++;
                }
                Graphics graphics5 = graphics2;
                SolidBrush brush3 = new SolidBrush(_TitleAreaColour);
                rect = new Rectangle(0, 0, Width, ItemHeight);
                graphics5.FillRectangle(brush3, rect);
                graphics2.DrawRectangle(new Pen(Color.FromArgb(35, 35, 35)), 1, 1, Width - 3, ItemHeight - 2);
                Graphics graphics6 = graphics2;
                string s = Text;
                Font font = new Font("Segoe UI", 10f, FontStyle.Bold);
                SolidBrush brush4 = new SolidBrush(_TextColour);
                rect = new Rectangle(0, 0, Width, ItemHeight + 2);
                graphics6.DrawString(s, font, brush4, rect, new StringFormat
                {
                    Alignment = StringAlignment.Center,
                    LineAlignment = StringAlignment.Center
                });
                graphics2.DrawRectangle(new Pen(Color.FromArgb(35, 35, 35), 2f), 1, 0, Width - 2, Height - 1);
                graphics2.DrawLine(new Pen(_BorderColour), 0, ItemHeight, Width, ItemHeight);
